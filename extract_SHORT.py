...
def extract_SHORT_in_specific(dat_collection,file_name,lead_num):
    
    SHORT_feature = pd.DataFrame(columns=['ShortStatWaveFeature_'+'lead_'+str(lead_num)+'_'+str(i) for i in range((2+5+16+2) * 6)])
    for j in range(len(dat_collection)):
        ...
        for each_R in range(1,len(rpeaks_indices_1[0])-1):
            ...
            fs = 300
            ts = WTfilt_1d(data['val'][lead_num])
            ini_point = R_P-90
        ...
        if  ',' not in record.__dict__['comments'][2][4:]:
            temp_SHORT_feature = pd.DataFrame(data = [[np.mean(QRS_peak_list), 
                                    ...
                                    ]],columns=['ShortStatWaveFeature_'+'lead_'+str(lead_num)+'_'+str(i) for i in range((2+5+16+2) * 6)])
            SHORT_feature = SHORT_feature.append(temp_SHORT_feature,sort=False)
            ...
            while len(s) != 0:
                ...
                SHORT_feature = SHORT_feature.append(temp_SHORT_feature,sort=False)
        print('finish'+str(j))
    SHORT_feature.to_csv(str(file_name)+'_Features/'+'12_lead_SF'+'/'+'SHORT_FEATURE'+str(lead_num)+'.csv', sep=',', header=True, index=False)
