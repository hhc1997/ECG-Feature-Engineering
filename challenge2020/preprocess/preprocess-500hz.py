...
dataDir = '/data/weiyuhua/data/Challenge2018/'
...
for i in range(len(files)):
    ...
    data = mat_data['ECG']['data'].squeeze()
    # Preprocessing
...
def getRpeakSet(ecgset, mode='0-1', index2del=[]):
    rpeakSet = np.zeros((len(ecgset), 30000))
    pwaveSet = np.zeros((len(ecgset), 30000))
    qwaveSet = np.zeros((len(ecgset), 30000))
    twaveSet = np.zeros((len(ecgset), 30000))
    for i in range(len(ecgset)):
      ...
      #      continue
        ...
        if i in index2del:
            continue
        # try:
        processed_ecg = nk.ecg_process(ecg[0:30000], sampling_rate=500)
...
def getSpecSet(ecgset):
    specSet = np.zeros((len(ecgset), 33, 500))
    for i in range(len(ecgset)):
        ecg = ecgset[i]
        f, t, spec = spectrogram(ecg, 500, nperseg=64, noverlap=0.5)
        spec = np.log(spec)
        for j in range(33):
...
# [1216, 1670, 2013, 2021, 2214, 2478, 2659, 2878, 3230, 5904, 6674]
...
#[592, 649, 863, 1389, 1580, 1670, 2006, 2799, 3203, 3497, 3842, 4004, 4080, 4133, 4652, 4787, 4841, 5189, 5618, 5828, 5841, 6230, 6696, 6850]
