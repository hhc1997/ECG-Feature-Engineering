...
from models.resnet import Classifier_RESNET_V2
from models.resnext import get_resnext34
...
def model_eval(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    # Load model
    model = Classifier_RESNET_V2(type=4, convfilt=16, ksize=64, depth=23, drop=0.2, head_dropout_rate=0.25).model
    ...
    callbacks = [
        ...
        AdvancedLearnignRateScheduler(monitor='val_loss', patience=6, verbose=1, mode='auto',
                                      decayRatio=0.1),
        ModelCheckpoint('./result/result-resnet34/best_model.hdf5',
                        monitor='val_loss', save_best_only=True, verbose=1),
        keras.callbacks.CSVLogger('./result/result-resnet34/log.csv', separator=',',
                                  append=True),
        TensorBoard(log_dir='./result/result-resnet34/tensorboard-log', write_graph=True, histogram_freq=1)
    ]
    ...
    model.load_weights('./result/result-resnet34/best_model.hdf5')
    ...
    import csv
    csvFile = open('./result/result-resnet34/result.csv', 'a', newline='')
    ...
    print(cvconfusion)
    scipy.io.savemat('./result/result-resnet34/cvconfusion.mat', mdict={'cvconfusion': cvconfusion.tolist()})
    return model
...
if __name__ == '__main__':
    ...
    model = model_eval(X_train, y_train, X_val, y_val, X_test, y_test)
    ...
    matfile = scipy.io.loadmat('./result/result-resnet34/cvconfusion.mat')
